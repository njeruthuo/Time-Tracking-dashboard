    ...
    <div className="bg-VeryDarkBlue text-white min-h-screen flex items-center justify-center">
      <div className="flex flex-col gap-6 sm:flex-row w-4/5 mx-auto">
        <div className="sm:w-[20%] bg-DarkBlue  rounded-lg">
          ...
          <div className="p-8 flex justify-between sm:flex-col">
            <p onClick={() => setSelect("daily")}>Daily</p>
            <p onClick={() => setSelect("weekly")}>Weekly</p>
            <p onClick={() => setSelect("monthly")}>Monthly</p>
          </div>
        </div>
        <div className="sm:w-[80%]  flex flex-col sm:flex-row flex-wrap space-y-2 place-items-center mb-4">
          {dataLoad.map((item, index) => {
            return (
              <div key={index} className="w-full sm:w-1/2 lg:w-1/3 ">
                <div
                  id={item.title}
                  className="sm:mx-4 border-DarkBlue border rounded-md"
                >
                  <div className="bg-DarkBlue mt-12 rounded-md px-4 pt-2">
                    ...
                    <div
                      id="info"
                      className=" mt-8   justify-between place-items-center sm:flex-col"
                    >
                      <h1 className="sm:text-3xl text-xl inline-block mr-32">
                        {item.timeframes[select].current}hrs
                      </h1>

                      <h3 className="sm:pb-8 inline-block">
                        {select === "daily"
                          ? `Last ${select.replace(
                              select[0],
